...
self-install will create a $HOME/.clj directory and initialize a local reposistory for Clojure packages in $HOME/.clj/lib. It will also create the shell and Windows batch scripts, $HOME/.clj/bin/clj and $HOME/.clj/bin/clj.bat, add $HOME/.clj/bin to your path,

@export PATH=~/.clj/bin:$PATH@

or move the scripts to a directory on your path.
