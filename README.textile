...
# Run @java -jar clj.jar self-install@ Note: 1) Requires java 1.6 and 2) if your browser added a .zip extension to the jar during download, just run @java -jar clj.jar.zip self-install@.
# Copy @~/.clj/bin/clj@ or @~/.clj/bin/clj.bat@ to your path
...
*Installation*
Download the <a href="http://incanter.org/downloads/clj.jar">clj.jar</a> file (or follow the instructions below to build from source):

@wget http://incanter.org/downloads/clj.jar@

and either double-click on the jar file to run self-install, or run the self-install command from the command line:

@java -jar clj.jar self-install@
