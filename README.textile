...
Clj uses Leiningen to manage the packages in the .cljr repository, and there is a project.clj file in the .cljr directory that can be customized by hand if desired.
...
*Install*
# Download <a href="http://incanter.org/downloads/cljr.jar">http://incanter.org/downloads/cljr.jar</a>
# Run @java -jar cljr.jar self-install@ Note: 1) Requires java 1.6 and 2) if your browser added a .zip extension to the jar during download, just run @java -jar cljr.jar.zip self-install@.
# Copy @~/.cljr/bin/cljr@ or @~/.cljr/bin/cljr.bat@ to your path
...
*Installation*
Download the <a href="http://incanter.org/downloads/cljr.jar">cljr.jar</a> file (or follow the instructions below to build from source):

@wget http://incanter.org/downloads/cljr.jar@

and either double-click on the jar file to run self-install, or run the self-install command from the command line:

@java -jar cljr.jar self-install@
...
@java -Duser.home=C:\ -jar cljr-1.0.0-SNAPSHOT-standalone.jar self-install@
...
@lein deps@
@lein uberjar@
@java -jar cljr-1.0.0-SNAPSHOT-standalone.jar self-install@
